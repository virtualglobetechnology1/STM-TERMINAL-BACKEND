...
def run_stm(
    ...
    # attach cumulative EC to df by date using last available event per day
    daily_ec = (
    ...
    # true EOD series from df, not trade log
    eod_account_value = (
...
def build_summary(
    ...
    """
    Build summary cards for frontend, including max drawdown stats.
    """
    if not equity_curve:
        return {
            "ticker": ticker,
    ...
    max_av_dd = min(float(x["av_dd"]) for x in equity_curve)
    max_av_dd_pct = min(float(x["av_dd_pct"]) for x in equity_curve)
    max_bh_dd = min(float(x["bh_dd"]) for x in equity_curve)
    max_bh_dd_pct = min(float(x["bh_dd_pct"]) for x in equity_curve)
    ...
    return {
        ...
        "max_av_dd": round(max_av_dd, 6),
        "max_av_dd_pct": round(max_av_dd_pct, 6),
        "max_bh_dd": round(max_bh_dd, 6),
        "max_bh_dd_pct": round(max_bh_dd_pct, 6),
        "total_trades": int(len(trade_log_df)),
    }
...
def run_backtest_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Main function to be called from the API layer.
    """
